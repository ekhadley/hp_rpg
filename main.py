...
from utils import *
import model_tools
from model_tools import Toolbox, roll_dice_tool_handler, read_story_file_tool_handler, list_story_files_tool_handler, read_cc_guide_tool_handler
...
def getFullInstructionMessage() -> str:
    ...
    with open("spellbook.md", "r") as file:
        spells = file.read()
    print(pink, list_story_files_tool_handler(), endc)
    full = f"```\n{ruleset}```\n\n```\n{spells}```\n\n```\n{narration_guide}```"
    return full
...
def loadIntoStory():
    ...
    if choice == 0:
        ...
        model_tools.current_story = stories[choice-1]
...
if __name__ == "__main__":
    ...
    dm_tb = Toolbox([roll_dice_tool_handler, read_story_file_tool_handler, list_story_files_tool_handler, read_cc_guide_tool_handler])
    asst = Assistant(
